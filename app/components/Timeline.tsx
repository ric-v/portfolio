...
const Timeline = ({ duration, title, company, companyLink, domain, desc, children }: Props) => {
  ...
  return (
    <>
      <div className={`col-span-3 lg:col-span-2 flex flex-row justify-center backdrop-blur-sm backdrop-brightness-95 rounded-2xl py-1 mx-1 hover:backdrop-brightness-75  dark:neumorphic-shadow hover:scale-110 cursor-pointer ${transition}`} onClick={() => setSelected(true)}>
      ...
      {selected && (
        <div className='fixed top-0 left-0 h-full flex flex-col justify-center items-center z-50 backdrop-blur-2xl backdrop-brightness-90' onClick={() => setSelected(false)}>
          <div className='rounded-2xl w-full md:w-2/3 p-2 md:p-10 mt-20'>
            <div className='text-center md:text-left overflow-y-scroll scrollbar-thin'>
              ...
              <div className={`text-xl md:text-2xl mt-2 font-medium text-slate-800 dark:text-sky-300 hover:font-bold hover:text-amber-700 hover:dark:text-sky-500 ${transition}`}>
